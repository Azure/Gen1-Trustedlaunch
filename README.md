...
3    |    Install `EFI Package`:<ul><li>**For Ubuntu**: `sudo apt install grub-efi-amd64`<br/>*Note*: `grub-efi-amd64-signed` is recommended if supported by OS configuration.<li>**For RHEL**: `sudo yum install gdisk grub2-x64-efi-modules efibootmgr dosfstools -y`</li></ul> | ![Ubuntu grub efi](./.attachments/01.On-Premise-Ubuntu.png)<br/>![RHEL grub efi](./.attachments/02.On-Premise-RHEL.png)
